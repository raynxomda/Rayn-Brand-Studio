...
export function Navbar() {
  ...
  return (
    <header className={`fixed left-0 right-0 top-0 z-50 px-5 py-5 transition-[background-color,backdrop-filter,border-color] duration-500 md:px-10 md:py-7 ${scrolled ? 'border-b border-white/10 bg-[#0B0B0C]/85 md:backdrop-blur-md' : 'border-b border-transparent bg-transparent'}`}>
      <nav className="mx-auto flex max-w-[1400px] items-center justify-between pb-4" aria-label="Main navigation">
          ...
          {links.map((link) => (
            <Magnetic key={link.href} strength={0.1}>
              <Link
                ...
                className={`relative block text-[.66rem] font-semibold uppercase tracking-[.17em] no-underline transition-colors duration-300 ${
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            className="mx-auto max-w-[1400px] overflow-hidden bg-[#141414] md:hidden"
